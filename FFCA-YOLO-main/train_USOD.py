...
def run_USOD_training():
    # 1. 基础配置解析
    opt = parse_opt(known=True)
    
    # 2. 针对 FFCA-YOLO 和 USOD 的核心参数修改
    # 注意：确保你的软链接 /data/USOD 已经配置好
    opt.data = str(ROOT / 'data/USOD.yaml') 
    ...
    # 路径与命名
    opt.project = str(ROOT / 'runs/train_USOD')
    opt.name = '2_FFCA_YOLO_USOD_1024'
    opt.exist_ok = True
    ...
    # 保存周期
    opt.save_period = 50        # 每10轮保存一次权重，防止意外中断
