...
class YOLO(object):
    ...
    #   检测图片
    # ---------------------------------------------------#
    def detect_image(self, image, save_path, crop=False, count=False):
        # ---------------------------------------------------#
        #   计算输入图片的高和宽
        # ---------------------------------------------------#
        ...
        for i, c in list(enumerate(top_label)):
            ...
            bbox = [int(left), int(top), int(right - left), int(bottom - top), float(score), int(c)+ 1, -1, -1]
...
if __name__=="__main__":
    ...
    for im_name in tqdm(im_list):
        ...
        save_path = os.path.join(save_dir, im_name.split(".jpg")[0]+".txt")
        image = Image.open(im_path)
        yolo.detect_image(image, save_path)
